...
Payment operations teams are putting renewed emphasis on resilience after Eskom implements Stage 4 load shedding on 2019-02-13.

Industry data shows that even modest processing delays can create a chain of downstream work: customer queries, retry decisions, finance reconciliation breaks and compliance evidence requests. In February 2019, comparable loan-payment files recorded 2 timeout-related rows, a level operations managers say is enough to justify daily monitoring.
...
The issue is becoming more urgent as customer counts grow. Analysts estimate that active retail banking customers increased by 7.6% year on year in the segment, increasing the volume of low-value transactions that still require control checks.
