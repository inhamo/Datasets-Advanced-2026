...
from commons.id_factory import make_customer_id
...
SANCTIONED_COUNTRY_CODES = ["IR", "KP", "SY", "CU", "RU", "MM", "BY"]

ID_TYPE_WEIGHTS = {
    "adult_sa": {"National ID": 0.86, "Driver's Licence": 0.12, "Passport": 0.02},
    "adult_foreign": {"Passport": 0.82, "Driver's Licence": 0.08, "Foreign ID": 0.10},
    "minor": {"Birth Certificate": 0.92, "Passport": 0.08},
}
...
COMMERCIAL_ADDRESS_BY_STATUS_RATE = {
    "Employed": 0.32,
    "Self Employed": 0.42,
    "Student": 0.08,
    "Pensioner": 0.02,
...
def generate_birth_certificate_number():
    ...
    return "BC" + "".join(str(random.randint(0, 9)) for _ in range(11))
...
def generate_drivers_licence_number():
    return "DL" + "".join(random.choice(string.digits) for _ in range(10))


def generate_foreign_id_number(country_code: str):
    return f"{country_code}-" + "".join(random.choice(string.digits) for _ in range(9))


def proper_email(full_name: str, domain: str | None = None):
    if random.random() >= 0.78:
        return None
    clean = "".join(ch.lower() if ch.isalnum() else "." for ch in full_name)
    clean = ".".join(part for part in clean.split(".") if part)
    suffix = random.choice(["", str(random.randint(10, 99)), str(random.randint(100, 999))])
    domain = domain or random.choice(["gmail.com", "outlook.com", "yahoo.com", "icloud.com", "webmail.co.za"])
...
def generate_customer_data(year, cadence="monthly", target_records=None, month=None):
    ...
    def generate_batch_individuals(batch_size_value, start_idx, used_ids_local=None):
        ...
        for i in range(batch_size_value):
            ...
            if not valid_occupations:
                valid_occupations = ["Unemployed"]

            valid_probs = np.array([occupation_lookup[occ]["weight"] for occ in valid_occupations], dtype=float)
            valid_probs = valid_probs / valid_probs.sum()
            employment_status = pick_from_distribution(EMPLOYMENT_STATUS_WEIGHTS)
            if employment_status == "Unemployed":
                occupations_batch[i] = "Unemployed"
            elif employment_status == "Student":
                occupations_batch[i] = "Student"
            elif employment_status == "Pensioner":
                occupations_batch[i] = "Pensioner"
            elif employment_status == "Self Employed":
                occupations_batch[i] = random.choice(["Spaza Shop Owner", "Self Employed Contractor", "Taxi Driver", "Street Vendor"])
            else:
                occupations_batch[i] = random.choices(valid_occupations, weights=valid_probs, k=1)[0]
        ...
        for i in range(batch_size_value):
            ...
            nationality = "South Africa" if random.random() < 0.94 else random.choices(non_sa_nationalities, weights=non_sa_weights, k=1)[0]
            citizenship = COUNTRY_CODES[nationality]
            dual_probability = 0.08 if citizenship != "ZA" else 0.012
            if random.random() < dual_probability:
                second = "ZA" if citizenship != "ZA" else random.choice(["ZW", "GB", "NG", "MZ", "LS", "BW"])
            ...
            id_profile = "adult_sa" if nationality == "South Africa" else "adult_foreign"
            id_type = pick_from_distribution(ID_TYPE_WEIGHTS[id_profile])
            if id_type == "National ID":
                id_number = generate_sa_id_number(correct_birth_date)
            elif id_type == "Driver's Licence":
                id_number = generate_drivers_licence_number()
            elif id_type == "Foreign ID":
                id_number = generate_foreign_id_number(COUNTRY_CODES[nationality])
            else:
                id_number = generate_passport_number()
            if random.random() < 0.05:
            ...
            email = proper_email(full_name)
            if email and random.random() < 0.025:
            ...
            customer_id = make_customer_id("individual", year, month, local_idx)
            ...
            if is_government_official and occupation in ["Student", "Unemployed"]:
            ...
            sanctioned_country_code = random.choice(SANCTIONED_COUNTRY_CODES) if random.random() < 0.004 else None
            sanctioned_country = sanctioned_country_code is not None
            ...
            customer_data = {
                ...
                "commercial_address": (
                    f"{fake.street_address()}, {city}, {province}, South Africa"
                    if random.random() < COMMERCIAL_ADDRESS_BY_STATUS_RATE.get(
                        "Unemployed" if occupation == "Unemployed" else "Student" if occupation == "Student" else "Pensioner" if occupation == "Pensioner" else "Self Employed" if occupation in ["Spaza Shop Owner", "Self Employed Contractor", "Taxi Driver", "Street Vendor"] else "Employed",
                        0.08,
                    )
                    else None
                ),
                ...
                "sanctioned_country_code": sanctioned_country_code,
    ...
    def generate_batch_companies(batch_size_value, start_idx, used_ids_local):
        ...
        for i in range(batch_size_value):
            ...
            if random.random() < 0.1:
                phone_number = mutate_phone_number(phone_number)
            customer_id = make_customer_id("organisation" if is_organisation else "company", year, month, idx)
            while customer_id in used_ids_local:
                idx += 1
                customer_id = make_customer_id("organisation" if is_organisation else "company", year, month, idx)
            used_ids_local.add(customer_id)
            ...
            company_sanctioned_country_code = random.choice(SANCTIONED_COUNTRY_CODES) if random.random() < 0.006 else None
            company_data = {
                "customer_id": customer_id,
                ...
                "visa_expiry_date": None,
                "is_pep": False,
                "sanctioned_country": company_sanctioned_country_code is not None,
                "sanctioned_country_code": company_sanctioned_country_code,
                "risk_score": risk_score,
    ...
    def generate_minor_accounts(adult_records, start_idx, used_ids_local):
        ...
        for parent in candidates[:target_minors]:
            ...
            customer_id = make_customer_id("minor", year, month, local_idx)
            while customer_id in used_ids_local:
                local_idx += 1
                customer_id = make_customer_id("minor", year, month, local_idx)
            used_ids_local.add(customer_id)
            ...
            minor_id_type = pick_from_distribution(ID_TYPE_WEIGHTS["minor"])
            ...
            minor_record = {
                ...
                "email": None,
                "phone_number": None,
                "id_type": minor_id_type,
                "id_number": generate_birth_certificate_number() if minor_id_type == "Birth Certificate" else generate_passport_number(),
                "expiry_date": None,
                "visa_type": None,
                "visa_expiry_date": None,
                "passport_expired": False,
